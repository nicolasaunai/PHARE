...
NO_DISCARD constexpr int centering2int(QtyCentering c) { return static_cast<int>(c); }
...
class GridLayout {
   ...
   public:
     ...
     */
    NO_DISCARD std::array<double, dimension> const& meshSize() const noexcept { return meshSize_; }
    ...
    NO_DISCARD auto indexesToCoordVectors(Indices const& indices, Centering const& centering,
        ...
        auto emplace_back = [](auto& vectors, auto const&& point) {
            ...
            if constexpr (dimension > 1) std::get<1>(vectors).emplace_back(point[1]);
            if constexpr (dimension > 2) std::get<2>(vectors).emplace_back(point[2]);
        };
        ...
        for (auto const& indiceTuple : indices)
            std::apply(
                [&](auto const&... args) {
                    emplace_back(xyz,
                                 this->indexesToCoords<WithField>(
     ...
     */
    NO_DISCARD std::uint32_t physicalStartIndex(QtyCentering centering, Direction direction) const {
    ...
    NO_DISCARD std::uint32_t static constexpr nbrGhosts(
        QtyCentering /*centering*/ =
            QtyCentering::primal) {  // Both dual and primal ghosts are the same!
    ...
    NO_DISCARD auto static constexpr nDNbrGhosts(
        Quantity /*centering*/ =
            QtyCentering::primal) {  // Both dual and primal ghosts are the same!
    ...
    NO_DISCARD auto deriv(Field const& operand, MeshIndex<Field::dimension> index) {
        ...
        if constexpr (Field::dimension == 1) {
            ...
            if constexpr (direction == Direction::X) {
                auto next = operand(nextIndex(fieldCentering[dirX], index[0]), index[1], index[2]);
                auto prev = operand(prevIndex(fieldCentering[dirX], index[0]), index[1], index[2]);
                return inverseMeshSize_[dirX] * (next - prev);
            }

            if constexpr (direction == Direction::Y) {
                auto next = operand(index[0], nextIndex(fieldCentering[dirY], index[1]), index[2]);
                auto prev = operand(index[0], prevIndex(fieldCentering[dirY], index[1]), index[2]);
                return inverseMeshSize_[dirY] * (next - prev);
            }
            if constexpr (direction == Direction::Z) {
                auto next = operand(index[0], index[1], nextIndex(fieldCentering[dirZ], index[2]));
                auto prev = operand(index[0], index[1], prevIndex(fieldCentering[dirZ], index[2]));
    ...
    NO_DISCARD auto laplacian(Field const& operand, MeshIndex<Field::dimension> index) {
        ...
        if constexpr (Field::dimension == 1) {
            ...
            return inverseMeshSize_[dirX] * inverseMeshSize_[dirX] * (nextX - 2.0 * hereX + prevX);
        }
            ...
            auto lapX =
                inverseMeshSize_[dirX] * inverseMeshSize_[dirX] * (nextX - 2.0 * hereX + prevX);
            ...
            auto lapY =
                inverseMeshSize_[dirY] * inverseMeshSize_[dirY] * (nextY - 2.0 * hereY + prevY);
            ...
            auto lapZ =
                inverseMeshSize_[dirZ] * inverseMeshSize_[dirZ] * (nextZ - 2.0 * hereZ + prevZ);
    ...
    NO_DISCARD static typename Field::type project(
        Field const& field, MeshIndex<dimension> index,
     ...
     * ghost nodes
     */
    NO_DISCARD std::array<std::uint32_t, dimension> nbrPhysicalNodes(
        Quantity::Scalar hybQty) const {
            ...
            if constexpr (dimension == 1) {
                ...
                for (auto iy = iy0; iy <= iy1; ++iy) {
                    if constexpr (dimension == 2) {
                        ...
                        for (auto iz = iz0; iz <= iz1; ++iz) fn(ix, iy, iz);
                    }
                }
            }
        }
